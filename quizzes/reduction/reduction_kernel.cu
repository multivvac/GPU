...
__global__ void reduction_naive_kernel(scalar_t *__restrict__ in,
  ...
  for (size_t stride = 1; stride <= blockDim.x; stride *= 2) {
    size_t idx = tid * stride * 2;
    if (idx < 2 * blockDim.x) {
      in[section + idx] +=
          (section + idx + stride) < N ? in[section + idx + stride] : 0;
    }
    __syncthreads();
  }
  if (tid == 0) {
...
torch::Tensor reduction_naive_cuda(torch::Tensor &data) {
  ...
  bool blockOverflow = ((N + 1) / 2) > MAX_THREAD_BLOCK;

  const size_t threads = blockOverflow ? MAX_THREAD_BLOCK : ((N + 1) / 2);
